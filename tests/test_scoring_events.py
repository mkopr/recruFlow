from app.scoring.events import ScoreEvent, publish_score, subscribe, unsubscribe


def test_publish_delivers_to_a_subscribed_queue() -> None:
    queue = subscribe()
    try:
        event = ScoreEvent(score_id=1, offer_id=2, title="t", company="c", score_percent=82)
        publish_score(event)
...
def test_unsubscribe_stops_further_delivery() -> None:
    queue = subscribe()
    unsubscribe(queue)
    publish_score(ScoreEvent(score_id=1, offer_id=2, title="t", company="c", score_percent=82))
