...
The pipeline dead letter queues (ingestion/scoring failures) store at most one row per failing resource (a job posting, a source's ingestion, an offer×profile pair), keyed by a `dedup_key`. A recurring failure upserts the existing row (`ON CONFLICT (dedup_key) DO UPDATE`) rather than appending a new one, and a successful retry flips it to `status="resolved"` in place instead of deleting it.
