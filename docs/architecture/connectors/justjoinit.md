...
- **Field mapping** (`map_justjoinit_offer`), from the confirmed list-item shape:
  ...
  | `seniority` | `experienceLevel` | Mapped to the shared canonical vocabulary via `app.ingestion.normalize.normalize_seniority` (P1US5) — see "Cross-connector schema consistency" in ../ingestion.md |
